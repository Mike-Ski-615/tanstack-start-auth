...
const { requestPasswordResetFn, resetPasswordFn } = await import(
  "../src/server/reset.functions"
);
...
describe.skipIf(!TEST_DATABASE_URL)("密码重置（集成测试）", () => {
  ...
  test("resetPasswordFn 成功 → 改密 + 旧会话全撤 + 自动登录", async () => {
    ...
    const { result, error, setCookieHeader: newCookie } = await callServerFn(
      resetPasswordFn,
    ...
    // 旧密码失效，新密码生效（login 会再次顶替，无妨——断言已完成）
    ...
    const { result: newPasswordLogin } = await callServerFn(login, {
      email,
