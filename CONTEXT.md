...
1. **ADR-0001 / ADR-0002 悬空** —— 两者都标为「已废弃」但没有指向替代决策。
   OTP 改动（URL token → 6 位数字）的取舍尚未记录：熵从 256-bit 降到 20-bit，
   以错误次数上限 + 双维度限速补偿。待补一条 ADR。
2. **重置密码是否顺带验证邮箱** —— 未决问题，见 ADR-0001。当前行为是不碰。
3. **`User.role` 只有 teacher / student** —— 无管理员角色，管理后台类需求需要先决定 |
