...
describe("registerSchema", () => {
  ...
  it("name 不能为空或超长", () => {
    ...
    expect(
      ok(registerSchema, { name: "x".repeat(51), email: "a@b.com", password: "123456" }),
    ).toBe(false);
  });
});
...
describe("verifyEmailOtpSchema / resetPasswordSchema 的 OTP 字段", () => {
  it("接受 6 位数字", () => {
    ...
    expect(ok(resetPasswordSchema, { email: "a@b.com", otp: "123456", password: "123456" })).toBe(
      true,
    );
  });
  ...
  it("resetPasswordSchema 同时要求 otp 和新密码", () => {
    ...
    expect(
      ok(resetPasswordSchema, { email: "a@b.com", otp: "123456", password: "12345" }),
    ).toBe(false);
  });
});
...
describe("changePasswordSchema", () => {
  ...
  it("新密码须符合长度要求", () => {
    expect(ok(changePasswordSchema, { currentPassword: "old123", newPassword: "12345" })).toBe(
      false,
    );
    expect(ok(changePasswordSchema, { currentPassword: "old123", newPassword: "123456" })).toBe(
      true,
    );
  });
});
